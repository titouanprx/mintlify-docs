...
For backward compatibility, the V2 API supports parameter aliases. The primary parameter name is `profile_url`, but `linkedin_url` is also accepted as an alias in all endpoints that take a profile URL. Both names work identically — use whichever you prefer.
