...
<Note>**WhatsApp**: send only the `account_id` to get the current QR code or `connected`; `resend: true` gives a new code once it expired. See [WhatsApp](/api-reference/v2/messages/whatsapp).</Note>
