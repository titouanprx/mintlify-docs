...
|---|---|---|
| `message_received` | `"message"` (LinkedIn) · `"message_received"` (email) | A new message / email was received |
...
<Warning>
  The subscription filter `message_received` is intentionally different from the actual `event.type` field, which is `"message"` for LinkedIn. Filter against `message_received` when creating the webhook, but check `event.type === "message"` when parsing the payload. For an **email** account the inner type is `"message_received"` (it matches the filter name) — see [Email events](#email-events).
</Warning>
...
**New email received** — inner `type` is `"message_received"` (not `"message"`):
