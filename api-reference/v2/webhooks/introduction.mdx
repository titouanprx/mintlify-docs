...
|---|---|---|
| `message_received` | `"message"` | A new message or email was received |
...
<Warning>
  The subscription filter `message_received` is intentionally different from the actual `event.type` field, which is `"message"`. Filter against `message_received` when creating the webhook, but check `event.type === "message"` when parsing the payload — the same for LinkedIn **and** email (see [Email events](#email-events)).
</Warning>
...
**New email received** — inner `type` is `"message"` (same as a LinkedIn message):
