---
title: "Search Companies"
api: "POST /v1/data/search/companies"
description: "Search LinkedIn company pages with advanced filters"
---

## Description

This endpoint allows you to search LinkedIn company pages using various advanced filtering criteria.

## Headers

<ParamField header="x-api-key" type="string" required>
  Your API key for authentication
</ParamField>

## Body Parameters

<ParamField body="keyword" type="string | array" required>
  Keyword(s) to search for (company name, industry, etc.)
  
  Can be a single string or an array of strings for batch searching.
  Can also be provided as `company_name` for backward compatibility
</ParamField>

<ParamField body="keywords" type="array">
  Alternative parameter for providing multiple keywords as an array
  
  Alias for `keyword` when using multiple search terms
</ParamField>

<ParamField body="industry" type="string | array">
  Industry sector(s) (e.g., "technology", "finance", "healthcare")
  
  Can be a single string or an array of industries
</ParamField>

<ParamField body="location" type="string | array">
  Location(s) (e.g., "Paris", "France", "New York")
  
  Can be a single string or an array of locations
</ParamField>

<ParamField body="company_size" type="string">
  Company size category (e.g., "startup", "small", "medium", "large", "enterprise")
</ParamField>

<ParamField body="employee_range" type="string">
  Employee count range (e.g., "1-50", "51-200", "201-1000", "1000+")
</ParamField>

<ParamField body="funding_stage" type="string">
  Funding stage (e.g., "seed", "series_a", "series_b", "series_c", "ipo")
</ParamField>

<ParamField body="founding_year" type="integer">
  Founding year (e.g., 2020)
  
  - Minimum: 1800
  - Maximum: 2024
</ParamField>

<ParamField body="founding_period" type="string">
  Founding period category (e.g., "startup", "recent", "established", "legacy")
</ParamField>

<ParamField body="num_results" type="integer" default="10">
  Number of results to return
  
  - Minimum: 1
  - Maximum: 50,000
  - Default: 10
</ParamField>

<ParamField body="country" type="string" default="FR">
  Country code for proxy (e.g., "FR", "US", "GB")
</ParamField>

<ParamField body="search_mode" type="string" default="single">
  Search mode type
  
  - "single": Single search with advanced filters (default)
  - "batch": Parallel search for multiple keywords
</ParamField>

## Response

<ResponseField name="status" type="string">
  Response status ("success" or "error")
</ResponseField>

<ResponseField name="data" type="object">
  <Expandable title="Response Data">
    <ResponseField name="results_returned" type="integer">
      Number of results returned in this response
    </ResponseField>
    
    <ResponseField name="companies" type="array">
      List of found companies
      <Expandable title="Company Structure">
        <ResponseField name="url" type="string">
          Company LinkedIn profile URL
        </ResponseField>
        <ResponseField name="title" type="string">
          Company title/name from LinkedIn
        </ResponseField>
        <ResponseField name="description" type="string">
          Company description
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="message" type="string">
  Error message (if status = "error")
</ResponseField>

## Request Example

```json
{
  "keyword": "technology",
  "industry": "technology",
  "location": "Paris",
  "company_size": "medium",
  "employee_range": "51-200",
  "funding_stage": "series_a",
  "founding_year": 2020,
  "founding_period": "recent",
  "num_results": 20,
  "country": "FR"
}
```

## Request Example with Arrays

```json
{
  "keywords": ["technology", "fintech", "startup"],
  "industry": ["technology", "finance", "healthcare"],
  "location": ["Paris", "Lyon", "Bordeaux"],
  "company_size": "medium",
  "employee_range": "51-200",
  "num_results": 30,
  "country": "FR",
  "search_mode": "batch"
}
```

## Response Example

<ResponseExample>
```json
{
  "status": "success",
  "data": {
    "results_returned": 10,
    "companies": [
      {
        "url": "https://uk.linkedin.com/company/crm.com",
        "title": "CRM.COM - LinkedIn",
        "description": "CRM.COM is a global specialist in designing and developing commerce, fintech and subscription-based solutions, ideal for the new generation digital consumers."
      },
      {
        "url": "https://www.linkedin.com/company/recruitcrm",
        "title": "Recruit CRM - LinkedIn",
        "description": "Recruit CRM builds cloud-based software for the global Recruitment & Staffing industry. We are on a mission to help recruitment firms grow faster with cutting ..."
      },
      {
        "url": "https://www.linkedin.com/company/aesthetix-crm",
        "title": "Aesthetix CRM - LinkedIn",
        "description": "The #1 CRM platform for plastic surgeons and medical spas. Aesthetix CRM gives your practice access to all the tools you need to grow."
      }
    ]
  }
}
```
</ResponseExample>

## Error Codes

<ResponseExample>
```json
{
  "status": "error",
  "message": "keyword (or company_name) is required"
}
```
</ResponseExample>

<ResponseExample>
```json
{
  "status": "error",
  "message": "founding_year must be a valid year between 1800 and 2024"
}
```
</ResponseExample>

- **400**: Invalid or missing parameters
- **401**: Invalid API key or insufficient credits
- **500**: Server error during request processing

## Important Notes

- Each successful search consumes 1 credit (single mode) or 1 credit per keyword (batch mode)
- Credits are only deducted on successful searches
- The endpoint uses an automatic retry system for proxy errors
- The `company_name` parameter is supported for backward compatibility with `keyword`
- **Single mode**: All filter parameters are optional - you can combine multiple filters for more precise results
- **Batch mode**: When using multiple keywords or `search_mode: "batch"`, searches are executed in parallel
- The `founding_year` parameter must be between 1800 and 2024
- Most search parameters support both single strings and arrays for flexible searching
- Use `keywords` parameter or `keyword` as array for batch searching multiple terms simultaneously

## Usage Examples

### Basic search with keyword only
```json
{
  "keyword": "artificial intelligence",
  "num_results": 10
}
```

### Search with industry and location filters
```json
{
  "keyword": "software",
  "industry": "technology",
  "location": "San Francisco",
  "num_results": 25
}
```

### Search with company size and funding filters
```json
{
  "keyword": "fintech",
  "company_size": "startup",
  "employee_range": "1-50",
  "funding_stage": "seed",
  "founding_period": "startup",
  "num_results": 15
}
```

### Batch search with multiple keywords
```json
{
  "keywords": ["fintech", "blockchain", "cryptocurrency"],
  "industry": ["Finance", "Technology"],
  "location": ["New York", "San Francisco", "London"],
  "search_mode": "batch",
  "num_results": 100
}
```

### Advanced search with multiple industries and locations
```json
{
  "keyword": "startup",
  "industry": ["Technology", "Healthcare", "Finance"],
  "location": ["Paris", "Lyon", "Marseille"],
  "company_size": "startup",
  "employee_range": "1-50",
  "num_results": 50
}
```