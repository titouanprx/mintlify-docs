...
<ParamField body="screening_questions" type="array">
  List of screening question objects to add to the job posting
</ParamField>

<ParamField body="screening_questions[0].type" type="string">
  Type of question. Can be a predefined LinkedIn question type or "CUSTOM_QUESTION"
</ParamField>

<ParamField body="screening_questions[0].questionText" type="string">
  Required for custom questions. The text of the question
</ParamField>

<ParamField body="screening_questions[0].format" type="string" default="MULTIPLE_CHOICE">
  For custom questions, the format of the question (currently supports "MULTIPLE_CHOICE")
</ParamField>

<ParamField body="screening_questions[0].required" type="boolean" default="true">
  Whether the question is required to be answered
</ParamField>

<ParamField body="screening_questions[0].parameterValue" type="string">
  For predefined questions, the value for the parameter (e.g., skill name, industry name)
</ParamField>

<ParamField body="screening_questions[0].parameterDisplayText" type="string">
  For predefined questions, the display text for the parameter
</ParamField>

<ParamField body="screening_questions[0].favorableFloor" type="number">
  For numeric questions, the minimum acceptable value (e.g., years of experience)
</ParamField>

<ParamField body="screening_questions[0].experienceYears" type="number">
  For PROFESSIONAL_EXPERIENCE questions, years of experience required
</ParamField>

<ParamField body="screening_questions[0].languageLevel" type="string">
...
- `INDUSTRY_EXPERIENCE`: Years of experience in a specific industry (requires `parameterValue` with industry name)
- `PROFESSIONAL_EXPERIENCE`: Years of general professional experience (requires `parameterValue` with field name)
- `SKILL_COMPETENCE`: Level of competence with a specific skill (requires `parameterValue` with skill name)
